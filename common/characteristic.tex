...
\textbf{Need for Next Generation EMS.}
...
\begin{itemize}
    \item Rapid change in power system operation challenges human operators in their supervision and control duties \cite{cigre_700}.
    ...
    \item Enhancing the visibility of the entire power system and improving scenario-based simulation are crucial for optimizing and securing electricity supply.

    \item The fast dynamics inherent in PEDGs, driven by the millisecond-level responses of IBR controllers, fall far below the observation window of traditional EMS. Events like fast transients, control interactions between IBRs, sub-synchronous oscillations, and rapid frequency deviations due to low inertia \cite{Orum2020} are simply invisible to conventional SE algorithms. Operating the system closer to its physical boundaries \cite{8295027}, as necessitated by the energy transition, means that understanding and predicting these fast dynamic phenomena becomes crucial for maintaining operational security.
\end{itemize}
...
% The aforementioned challenges in microgrid operation and control alter the requirements for EMS as well.  


% The challenges outlined in Section 1.1 fundamentally alter the requirements for power system state awareness.
